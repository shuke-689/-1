...
2. 找关闭按钮（`FIND_CLOSE_JS`）三种依据，优先级 `aria-label/title 写关闭` >
   `class 名带 close` > `文本本身是个 ✕ 符号`；同级按**面积升序**
   （关闭按钮一定比外层容器小），尺寸限定 8~90 px。
3. 点掉后复查文案是否消失；没消失就 `Esc` 兜底，最多 3 轮。
4. **任何失败都不抛异常**，关不掉照样读抖音号（顶部区域本来就不被挡），只留日志。
