...
description: 抖音精选联盟（巨量百应）达人筛选采集 + 微信自动加好友全链路自动化。覆盖按主推类目/直播结算总额/粉丝量筛选带货达人、进达人主页核查「带货分析」（禁忌商品/低价铺货/店铺家数/同品牌占比四条规则）、采集微信号、写入 Excel 登记表，以及通过微信桌面端 OCR+键鼠注入自动发送好友申请（含多轮台账与风控熔断）。当用户说「开始筛选达人」「采集达人」「找达人」「跑达人」「开始帮我添加微信」「加微信好友」「继续加微信」时使用；也用于排查该链路的登录态失效、平台限流 11001、浏览器假死、类目按钮定位失败等问题，以及在多台机器/多人之间同步规则与脚本。
agent_created: true
---
...
"$PY" .probe/test_new_rules.py  # 禁忌词扩展 + 店铺家数 + 低价铺货 + 价格解析（37 组）
```
