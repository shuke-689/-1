...
> 已登录就直接跑；没登录就**自动在浏览器里新开一个标签页打开官方登录页**
> （`https://buyin.jinritemai.com/login`），并在日志里打印「当前未登录精选联盟」，
> 然后**停在原地等你**。你只要在**它打开的那个 Edge 窗口**里登录一下
> （扫码 / 账号密码都行），检测到就自动继续。会话存进 `.edge-auto/profile`，
> 以后长期有效。
>
> ⚠️ 必须在**它打开的那个 Edge 窗口**里登——在你自己平时用的浏览器里登没用。
>
> 等太久没登就调大 `LOGIN_WAIT_SEC`（默认 600 秒）：
>
> ```bash
> LOGIN_WAIT_SEC=7200 "$PY" collect_30.py
> ```
>
